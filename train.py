...
def swan(inputs, n_blocks=1, n_heads=4, head_size=64, context=10, out_dim=128):
...
dg_train = dg_nlvocoder('wave.wav', 'excitation.wav', batch_size = 16)
regularizer = tf.keras.regularizers.L2(0)
initializer = tf.keras.initializers.GlorotUniform() 

if os.path.exists('mfcc_mean.npy'):
    mean = np.load('mfcc_mean.npy')
    variance = np.load('mfcc_var.npy')
else:
    #Get Normalization
    x=[]
    for i in range(400):
    ...
    np.save('mfcc_var.npy', variance)

Normalizer = tf.keras.layers.experimental.preprocessing.Normalization(
        axis=-1, mean=mean, variance=variance)

mfcc = tf.keras.Input(shape=(None,60), dtype=tf.float32)
exct = tf.keras.Input(shape=(None,400), dtype=tf.float32)

norm_mfcc = Normalizer(mfcc)
mfc = tf.keras.layers.Dense(128, activation='linear')(norm_mfcc)
mfc = tf.keras.layers.Dense(256, activation='linear')(mfc)
filtr = swan(mfc, n_blocks=4, n_heads=5, head_size=80, context=12, out_dim=400)
...
est_wav = tf.keras.layers.Lambda(lambda z: overlap_and_add(z, frame_length=400, frame_step=80))(convolved)
...
model_check = ModelCheckpoint('weights/weights-{epoch:04d}.h5', monitor='loss', save_best_only=True)
